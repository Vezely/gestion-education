import styles from '../../styles/commune.module.css';
...
const Ville = () => {
	...
	useEffect(() => {
		async function fetchCommuneData() {
			if (nomCommune) {
				const response = await fetch(`/api/commune?nom_commune=${nomCommune}`, {
					method: 'GET',
	...
	const onSubmit = async (data) => {
		let response = await fetch(`/api/etablissements?nom_commune=${nomCommune}`, {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
		...
		if (response.ok) {
			const etablissementData = await response.json();
			setEtablissementData(etablissementData);
		}
	};
	...
	return (
		<div className={styles.container}>
			<div className={styles.contenu}>
				...
				<h1>
					{nomCommune.charAt(0).toUpperCase() + nomCommune.slice(1)} <span></span>
				</h1>
				<p>{communeData[0]?.description}</p>
				<div className={styles.informations}>
					<h2>Decouvrez les établissements scolaires de la ville de {nomCommune}</h2>
					<div className={styles.bar_recherche}>
						<form onSubmit={handleSubmit(onSubmit)}>
							<label htmlFor='recherche'>
								<input
									{...register('nom_etablissement', { required: false })}
									type='search'
									name='nom_etablissement' // Attribut name mis à jour
									placeholder='Trouvez des établissements ici'
					...
					<div className={styles.contenair_box}>
						{etablissementData.length > 0 &&
							etablissementData.map((etablissement) => (
								<div className={styles.box_ecole} key={etablissement.id_etablissement}>
									<Link href={`/etablissement/${etablissement.nom_etablissement.replace(/ /g, '_')}?Commune=${nomCommune}`}>
										<div>
						...
						{etablissementData.length == 0 && <div className={styles.aucunResultat}>Aucun résultat trouvé</div>}
					</div>
					<div className={styles.voirPus}>
						<div>Voir plus</div>
					</div>
					<Communes nomCommune={nomCommune} />
				</div>
			</div>
		</div>
	);

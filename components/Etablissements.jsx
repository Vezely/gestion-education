import styles from '../styles/Etablissements.module.css';
...
const Etablissements = () => {
	...
	useEffect(() => {
		...
		if (etablissement) {
			setNomEtablissement(etablissement.etablissement);
		}
	}, [router.query]);

	useEffect(() => {
		async function fetchEtablissementsData() {
			const response = await fetch(`/api/allEtablissements?nomEtablissement=${router.query.Commune}`, {
				method: 'GET',
	...
	return (
		<div className={styles.informations}>
			<div className={styles.contenair_box}>
				{etablissementData.map((etablissement) => (
					<div className={styles.box_ecole} key={etablissement.id_etablissement}>
						<Link href={`/etablissement/${etablissement.nom_etablissement.replace(/ /g, '_')}`}>
							<div>
								<h3>{etablissement.nom_etablissement}</h3>
								<div className={styles.infos}>
									<p>{tronquerTexte(etablissement.adresse, 20)}</p>
									<div>{etablissement.nom_type}</div>
								</div>
								<div className={styles.niveau}>
									<div className={styles.eleves}>
										...
										<span>{etablissement.nombre_eleves}</span>
									</div>
									<div className={styles.etoils}>
										<div className={styles.eVide}>★★★★★</div>
										<div className={styles.ePlein}>★★★★</div>
									</div>
								</div>
							</div>
						</Link>
